...
export const Payment = ({ButtonClick}: Props) => {
    ...
    const formik = useFormik<Order>({
        ...
        onSubmit: value => {alert(JSON.stringify(value, null, 2)); request(value)}

    })
    const [paymentInformationState, setPaymentInformationaState] = useState(false)
    const [request, {data: response, isSuccess: isSucessRequest, isError: isError}] = usePostPaymentMutation()
    return (
    <>
